...
TrackingConfidence TrackingConfidenceConvert(PxrTrackingConfidence confidence)
{
    switch (confidence)
    {
    case PxrTrackingConfidence_LOW:
        return TrackingConfidence_Low;
        break;
    case PxrTrackingConfidence_HIGH:
        return TrackingConfidence_High;
        break;
    }
}
...
{
    handStateInternal->Status = HandTracked; //*((HandStatus *)&aimState.Status);
    ...
    handStateInternal->PointerPose = PxrPosefFlipped(aimState.aimPose);
